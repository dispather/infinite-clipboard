...
**추가 확인(2026-07-12 후속 질문에 답하며 코드로 재확인)**: 이 자동 수신은
`download_path`/Downloads 로 바로 안 갑니다. `_fetch_offer` 가 반환하는
경로는 OS 임시 디렉토리(`<시스템 temp>/ic_transfer_<transfer_id>/`, macOS
기준 `/var/folders/.../T/...`)이고, 클립보드엔 이 임시 경로가
`public.file-url` 로만 등록됩니다. 사용자가 보는 폴더에 파일이 실제로
생기는 건 **진짜 Cmd+V 로 Finder 자신이 임시→목적지 복사를 수행할 때뿐**
입니다. 그래서 peek 만 있고 진짜 paste 가 없었던 경우 잔여 위험은 정확히는
"대역폭 낭비 + `staging_ttl_hours`(기본 24시간) 동안 임시 폴더에 파일이
존재"이지, "복사만 해도 Downloads 에 파일이 쌓인다"는 아닙니다 — 처음
드린 설명보다는 덜 심각한 수준이라 정정합니다.
