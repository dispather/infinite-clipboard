...
to_host: windows (sh-knu)
topic: infinite-clipboard-peekabooWin-verification-tooling
created: 2026-07-12
---

# [linux → sh-knu] Windows 실기 UI 검증용 PeekabooWin 설치 요청 (급하지 않음)
...
같은 계열(FelixKruger/PeekabooWin)이 Windows 전용으로도 있습니다:
https://github.com/FelixKruger/PeekabooWin — Windows 친화적으로 포팅된 버전.

## 요청

Windows는 이 리눅스 세션이 직접 다룰 수 없는 플랫폼이라(에이전트가 리눅스에서만 Bash를
실행함), sh-knu에서 도는 세션/사용자분께서 여유 있으실 때:

1. `PeekabooWin`(FelixKruger/PeekabooWin) 설치
2. 이 머신의 infinite-clipboard 로컬 체크아웃에서 실행 중인 GUI(설정창/전송창/히스토리창
