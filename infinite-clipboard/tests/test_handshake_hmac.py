...
def test_challenge_roundtrip():
    p = Protocol("k")
    sn = generate_nonce()
    wire = p.create_handshake_challenge(sn)
    ...
    assert payload == {"server_nonce": sn, "server_version": PROTOCOL_VERSION}


def test_challenge_rejects_invalid_nonce():
    p = Protocol("k")
    with pytest.raises(ValueError):
        p.create_handshake_challenge("not-a-nonce")
...
def test_response_roundtrip():
    ...
    wire = p.create_handshake_response(cn, "MyDevice", h)
    parsed = p.parse_message(wire[4:])
...
def test_response_rejects_invalid_inputs():
    ...
    h = "a" * 64
    with pytest.raises(ValueError):
        p.create_handshake_response("bad-nonce", "name", h)
    with pytest.raises(ValueError):
        p.create_handshake_response(cn, "x" * 300, h)
    with pytest.raises(ValueError):
        p.create_handshake_response(cn, "name", "short")
...
def test_parse_response_rejects_garbage():
    ...
    # 누락 필드
    for missing in ("client_nonce", "client_version", "name", "hmac"):
        d = dict(valid)
        d.pop(missing)
...
@pytest.mark.parametrize("remote,compatible", [
    ("2.2", True),
    ("2.2.0", True),    # patch 차이는 호환 (major.minor 만 비교)
    ("2.2.99", True),
    ("2.1", False),     # major.minor 다름 — hard break
    ("2.1.0", False),
    ("2.0.0", False),
    ("3.0", False),
    ("1.9", False),
